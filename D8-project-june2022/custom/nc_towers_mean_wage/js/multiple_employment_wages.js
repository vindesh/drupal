(function ($, Drupal, drupalSettings) {
  ...
  $(document).ready(function() {
    ...
    function wagesDataTableLoad(){
        ...
        var groupClass = $("select#meanWagesMultiSearchFilter").find("option:selected").attr('data-groupclass');
    ...
    if ($("#employmentWagesGraphData").children().length > 0) {
        ...
        function afterYearFilter(chart) {
            ...
            selectEle.on("change", function() {
                ...
                if (chart) {
                    ...
                    $(".multiWages .graph-container__filters-dropdown ul li input").off("change");
                    hideGenderRacefilters();

                    var selectedGroup = $('#meanWagesMultiSearchFilter').find(":selected").attr("data-groupclass");
        ...
        function renderFilterValues() {
            ...
            $.each( items, function( i, row ) {    
                selectEle.append(
                        "<option data-groupclass=" + row.class + " value=[data-val='" + row.class + "']>" + row.value + "</option>"
                    );  
            });
        }
        renderFilterValues();
